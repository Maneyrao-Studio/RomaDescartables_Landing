...
export async function getProducts(): Promise<Product[]> {
  try {
    return await productService.getProductsForCatalog()
  } catch (error) {
    console.error('Error fetching products from database, falling back to static data:', error)
...
export async function getProductById(id: string): Promise<Product | null> {
  try {
    return await productService.getProductForCatalog(id)
  } catch (error) {
    console.error('Error fetching product from database, falling back to static data:', error)
    return getStaticProducts().find(p => p.id === id) || null
  }
}

export const PRODUCTS: Product[] = getStaticProducts()
